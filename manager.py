...
parser.add_argument("--task-num", type = int, default = None, metavar = 'Task Number')
parser.add_argument("--class-per-task", type = int, default = None, metavar = 'Class per Task')
                            ...
                            for tau in tau_all:
                                for neuron in neuron_model:
                                    for s, seedi in enumerate(seed_all):
                                        ...
                                        if dataset == 'cifar100':
                                            cmd+=' --task-num 19 ' if args.task_num is None else f' --task-num {args.task_num} '
                                            cmd+=' --in-size 768 '
                                            cmd+=' --task-sequence 5 ' if args.class_per_task is None else f' --task-sequence {args.class_per_task} '
                                            cmd+=' --class-per-task 5 ' if args.class_per_task is None else f' --class-per-task {args.class_per_task} '
                                            cmd+=' --batch-size 128 '
                                            cmd+=' --out-size 100 '
                                            cmd+=' --wins 10 '
